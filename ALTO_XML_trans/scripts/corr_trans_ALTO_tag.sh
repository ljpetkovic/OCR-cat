...
function dpi_and_transform {
	...
	do 
		python3 $path_ALTO_XML/scripts/corr_trans_ALTO_tag.py $f $dpi # transform those files with respect to their resolution
		echo "Processing $f" 
	done	
}
...
function suppr_transformed {
	liste_trans=$(find $1  -type f -name "*trans1.xml") # find all the files ending with 'trans.xml'
...
if [ "${option_total}" == "a" ]; then 
	...
	do
		if compgen -G "$g/*_trans1.xml" > /dev/null; then # check whether the file ending with '_trans.xml' already exists
